      ...
      const promise = new Promise<PageTexture>((resolve, reject) => {
        ...
        loader.load(
          ...
          (texture) => {
            ...
            console.log(
              `🔄 Loading progress: ${Math.round(progress * 100)}% (${index + 1}/${this.options.images.length})`,
            )
          ...
          (error) => {
            ...
            reject(
              new Error(
                `Failed to load ${imagePath}: ${error instanceof Error ? error.message : 'Unknown error'}`,
              ),
            )
          },
        )
  ...
  public nextPage(): void {
    ...
    const timeline = gsap.timeline({
      onComplete: () => {
        ...
        if (
          currentPageMesh.material instanceof THREE.ShaderMaterial &&
          currentPageMesh.material.uniforms?.uBend
        ) {
  ...
  public prevPage(): void {
    ...
    const timeline = gsap.timeline({
      onComplete: () => {
        ...
        // Reset bend values (only for shader materials)
        if (
          prevPageMesh.material instanceof THREE.ShaderMaterial &&
          prevPageMesh.material.uniforms?.uBend
        ) {
